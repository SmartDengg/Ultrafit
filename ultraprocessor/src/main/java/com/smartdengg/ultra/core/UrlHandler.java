...
public class UrlHandler extends UltraHandler<Annotation[]> {
    ...
    @Override
    void apply(RequestBuilder builder, Annotation[] annotations) {
        ...
        for (Annotation httpAnnotation : annotations) {
            ...
            if (this.restType != null) {
                String previousUrl = Reflections.invokeMethod(httpAnnotation, clazz, RequestBuilder.HTTP_METHOD);
                throw Utils.methodError(CLASS, "Only one HTTP method is allowed!\n Found: %s: '%s' or %s: '%s'!", this.restType, this.url, restMethod
                        .type(), previousUrl);
            }
        ...
        if (this.restType == null || this.url == null) {
            throw Utils.methodError(CLASS, "Http method annotation is required (e.g.@HttpGet, @HttpPost, etc.).");
        }
