...
public abstract class UseCase<R, S> {
    ...
    @SuppressWarnings("unchecked")
    public void subscribe(final R requestEntity, Observer<S> useCaseSubscriber, final Action1<RequestEntity> action) {
    ...
    //@formatter:off
    @SuppressWarnings("unchecked")
    public void subscribe(final R requestEntity, final Action1<? super S> onSuccess, final Action1<Throwable> onError, final Action1<RequestEntity> action) {
        ...
        this.subscription = Single.defer(new Func0<Single<RequestEntity>>() {
                                      @Override
                                      public Single<RequestEntity> call() {
                                          return UltraParserFactory.createParser(requestEntity)
                                                                   .parseRequestEntity()
                                                                   .as(Single.class);
                                      }
                                  ...
                                  .map(new Func1<RequestEntity, RequestEntity>() {
                                      @Override
                                      public RequestEntity call(RequestEntity requestEntity) {
                                          if (action != null) {
                                              action.call(requestEntity);
                                          } else {
                                              Logger.t(Constants.OKHTTP_TAG, 0)
                                  ...
                                  .flatMap(new Func1<RequestEntity, Single<S>>() {
                                      @Override
                                      public Single<S> call(RequestEntity requestEntity) {
